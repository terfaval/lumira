...
export default async function FortunePage({ searchParams }: FortunePageProps) {
  ...
  const requestedView = Array.isArray(rawView) ? rawView[0] ?? null : rawView ?? null;
  ...
  let initialJournalSessions: FortuneJournalSessionRecord[] = [];
  let recoveryError: string | null = null;
  let initialMode = null;
  const initialLandingView = requestedView === "journal" && !sessionId ? "journal" : "library";
  ...
  } else if (initialLandingView === "journal") {
    initialJournalSessions = await repository.listStartedSessionsForJournal(userId);
  }
  ...
  return (
    <FortuneJournalingPageClient
      ...
      modes={modes}
      initialLandingView={initialLandingView}
      initialJournalSessions={initialJournalSessions}

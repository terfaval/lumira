...
export async function POST(request: Request, context: RouteParams) {
  ...
  if (parsed.value.source === "reflective_space_surface") {
    return NextResponse.json(
      { error: "Reactivation requires explicit revisit source." },
      { status: 400 },
    );
  }

  const repository = createOpeningRepository();
  const opening = await repository.reactivateOpening(parsed.value);
  if (!opening) {

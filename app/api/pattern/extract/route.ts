...
export async function POST(req: Request) {
  try {
    ...
    const dreamText = sanitizeText(session.raw_dream_text ?? "");
    if (dreamText.length < MIN_DREAM_LEN) {
      ...
      if (upsertError) {
        console.warn("pre: upsert empty observation failed", upsertError.message);
      }
      return NextResponse.json({ ok: true, observation: compactDreamObservation(empty), too_short: true });
    }
    ...
    try {
      ...
      console.warn("pre: model call failed", error);
      return NextResponse.json({ ok: false, error: "Model call failed" }, { status: 200 });
    }

    const observation = parseDreamObservation(observationRaw);
    if (!observation) {
      console.warn("pre: invalid observation payload", observationRaw);
      return NextResponse.json({ ok: false, error: "Invalid observation schema" }, { status: 200 });
    }
    ...
    if (upsertError) {
      console.warn("pre: upsert observation failed", upsertError.message);
      return NextResponse.json({ ok: false, error: "Observation write failed" }, { status: 200 });
    }

    return NextResponse.json({ ok: true, observation: compactDreamObservation(observation) });
  } catch (error) {
    console.warn("pre: handler exception", error);

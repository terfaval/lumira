...
create or replace view public.dream_session_summaries_ui as
select
  session_id,
  user_id,
  title,
  ...
  created_at,
  updated_at
from public.dream_session_summaries;

-- A view a base table RLS policy-jait örökli (invoker).
-- Ha később szeretnéd szigorítani, lehet külön policy/view stratégia is.

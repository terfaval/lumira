...
begin
  ...
  cleaned := pg_catalog.regexp_replace(public.unaccent(pg_catalog.lower(raw)), '[^a-z0-9]+', ' ', 'g');
  tokens := pg_catalog.regexp_split_to_array(cleaned, '\\s+');

  foreach token in array tokens loop
    if token is null or pg_catalog.length(token) < 3 then
      continue;
    end if;
    if token = any(stopwords) then
      continue;
    end if;
    out_tokens := pg_catalog.array_append(out_tokens, token);
  end loop;

  return pg_catalog.array_to_string(out_tokens, ' ');
end;
$$;
...
begin
  ...
  set
    ...
    latent_analysis_log =
      pg_catalog.coalesce(latent_analysis_log, '[]'::jsonb)
      || pg_catalog.jsonb_build_array(
           pg_catalog.jsonb_build_object(
             'ts', pg_catalog.now(),
             'output', p_output,
             'meta', pg_catalog.coalesce(p_meta, '{}'::jsonb)
           )
         )
